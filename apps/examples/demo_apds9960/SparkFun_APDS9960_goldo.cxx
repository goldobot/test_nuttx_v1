...
bool SparkFun_APDS9960::init()
{
    ...
    if( !wireReadDataByte(APDS9960_ID, id) ) {
        printf( "wireReadDataByte(APDS9960_ID) failed \n" );
        return false;
    }
    printf( "wireReadDataByte(APDS9960_ID) : id=%x \n", id );
...
bool SparkFun_APDS9960::wireWriteDataByte(uint8_t reg, uint8_t val)
{
  ...
  msg[0].addr      = APDS9960_I2C_ADDR;
  msg[0].flags     = 0;
  msg[0].buffer    = i2c_buf;
  msg[0].length    = 2;

  ret = i2cdev_transfer(i2c_fd, msg, 1);
...
bool SparkFun_APDS9960::wireReadDataByte(uint8_t reg, uint8_t &val)
{
  ...
  msg[1].addr      = APDS9960_I2C_ADDR;
  msg[1].flags     = I2C_M_READ;
  msg[1].buffer    = i2c_buf;
  msg[1].length    = 1;

  if (I2C_RESEND_START) {
    ret = i2cdev_transfer(i2c_fd, &msg[0], 1);
    if (ret==NUTTX_OK) {
      ret = i2cdev_transfer(i2c_fd, &msg[1], 1);
    }
  } else {
    ret = i2cdev_transfer(i2c_fd, &msg[0], 2);
  }
